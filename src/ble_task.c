...
#define HIDD_DEVICE_NAME "HackyLapse"

// Button to take a picture.  
#define SHUTTER_KEY HID_CONSUMER_VOLUME_UP
...
static void tick_timer(void *arg)
{
	...
	// Take one snapshot if we have to
	if(takeSnapshot) {
		takeSnapshot = false;
		esp_hidd_send_consumer_value(hid_conn_id, SHUTTER_KEY, true);
		esp_hidd_send_consumer_value(hid_conn_id, SHUTTER_KEY, false);
	}

	// Handle normal countdown state
	if(getState() == STATE_RUNNING)
	{
		// Ticks are every 100ms
		countdown -= 100;
		if (countdown <= 0)
		{
			// Reset timer
